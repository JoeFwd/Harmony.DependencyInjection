...
internal sealed class PatchApplier : IPatchApplier
{
    ...
    /// </summary>
    /// <param name="patches">
    ///     The collection of patches to apply. If <c>null</c>, an <see cref="ArgumentNullException" /> is
    ///     thrown.
    /// </param>
    /// <param name="harmony">
    ///     The Harmony instance used for patching. If <c>null</c>, an <see cref="ArgumentNullException" />
    ///     is thrown.
    /// </param>
    public void Apply(IEnumerable<IPatch> patches, HarmonyLib.Harmony harmony)
    {
        ...
        lock (harmony)
        {
            foreach (var patch in patches)
            {
                ...
                {
                    _logger.LogWarning("Patch {Patch} is missing TargetMethod or PatchMethod.",
                ...
                try
                {
                    ...
                    _logger.LogInformation("Successfully applied {Patch} ({PatchType}).", patch.GetType().FullName,
                        patch.PatchType);
                }
                catch (Exception ex)
                {
